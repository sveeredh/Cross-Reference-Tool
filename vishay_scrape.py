...
def _parse_vishay_row(part_row, source_df_name, part_number):
    """
    Parses a row from the vishayicroelectronics CSV datasheet to extract key parameters.
    """
    specs_result = {
        ...
        "IEC 61000-4-2": "-", # Not present in the vishayicroelectronics CSV header
        "Capacitance": "-",
...
def fetch_vishay_specs_from_excel(part_number, vishay_specs_df):

    if part_number is None or vishay_specs_df is None or vishay_specs_df.empty:
        return None
    ...
    for col in vishay_specs_df.columns:
    ...
    if mfr_part_num_col:
        match_row_series = vishay_specs_df[vishay_specs_df[mfr_part_num_col].astype(str).str.strip().str.lower() == part_number.lower()]
        
        if not match_row_series.empty:
            part_row = match_row_series.iloc[0]
            matched_part_number = part_row[mfr_part_num_col]
            return _parse_vishay_row(part_row, "vishayicroelectronics_specs.csv", matched_part_number)
    ...
    if dk_part_num_col:
        ...
        match_row_series_dk = vishay_specs_df[
            vishay_specs_df[dk_part_num_col].astype(str).str.contains(search_pattern, case=False, na=False, regex=True)
        ]
        ...
        if not match_row_series_dk.empty:
            ...
            return _parse_vishay_row(part_row, "vishayicroelectronics_specs.csv", display_part_number)

    # If both searches fail, print the final message
    print(f"Part '{part_number}' not found in the local vishayicroelectronics file by Mfr P/N or DK P/N.")
    return None