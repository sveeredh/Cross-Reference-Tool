...
def _classify_digikey_package(supplier_pkg, case_pkg):
    ...
    is_0603_token = bool(re.search(r'(^|[^0-9])0603([^0-9]|$)', sup)) or _has_dim(sup, "SOD882", "SOD-882", "SOD 882")
...
DIODES_ZENER_DATABASE_FILE = "diodes_specs_zener.xlsx"
DIODES_MORE_ZENER_DATABASE_FILE = "diodes_specs_more_zener.xlsx"
DIODES_MORE_MORE_ZENER_CSV_DATABASE_FILE = "diodes_specs_more_more_zeners.csv"
DIODES_MORE_MORE_ZENER_DATABASE_FILE = "diodes_specs_more_more_zeners.xlsx"
...
def is_similar_voltage(v1_str, v2_str, tolerance_percent=20):
    ...
    return abs(v1 - v2) / ((v1 + v2) / 2) * 100 <= tolerance_percent
...
def find_ti_zener_alternatives(competitor_specs, ti_zener_specs_df):
    ...
    _, package_matches_df = find_package_matches(comp_pkg_alias, ti_zener_specs_df)
...
def find_ti_alternatives(competitor_specs, ti_specs_df):
    ...
    stage1_package_matches_df, final_package_matches_df = find_package_matches(comp_pkg_alias, base_ti_df_for_search)
    ...
    if comp_vrw_numeric is not None and 'Vrwm (V)' in base_ti_df_for_search.columns:
        vrwm_matched_indices = base_ti_df_for_search['Vrwm (V)'].apply(
            lambda ti_vrwm: is_similar_voltage(comp_vrw_str, str(ti_vrwm), tolerance_percent=30)
        )
    ...
    for index, ti_row in target_df_for_params.iterrows():
        ...
        if comp_vrw_numeric is None:
            ...
            vrw_filter_passed = is_similar_voltage(comp_vrw_str, ti_vrw_str_excel, tolerance_percent=30)
        ...
        if is_pkg_match_for_score: score += 4500
        ...
        if meets_min_criteria_relaxed and score > 0:
            potential_alternatives.append({
                ...
                "is_package_match": is_pkg_match_for_score, "exact_vrw_match": exact_vrw_match,
...
def main():
    ...
    diodes_more_zener_df = load_and_cache("diodes_more_zener.pkl", DIODES_MORE_ZENER_DATABASE_FILE, force_reload,
        lambda: load_excel_data(DIODES_MORE_ZENER_DATABASE_FILE, header_row=0))
    ...
    diodes_more_more_zener_df = load_and_cache("diodes_specs_more_more.pkl", DIODES_MORE_MORE_ZENER_DATABASE_FILE, force_reload,
        lambda: pd.read_excel(DIODES_MORE_MORE_ZENER_DATABASE_FILE))
    if diodes_more_more_zener_df.empty and os.path.exists(DIODES_MORE_MORE_ZENER_CSV_DATABASE_FILE):
        print(f"First-time setup: Found '{DIODES_MORE_MORE_ZENER_CSV_DATABASE_FILE}'. Converting...")
        if _unblock_csv_file(DIODES_MORE_MORE_ZENER_CSV_DATABASE_FILE):
            temp_df = pd.read_csv(DIODES_MORE_MORE_ZENER_CSV_DATABASE_FILE, on_bad_lines='skip', encoding='latin-1')
            temp_df.to_excel(DIODES_MORE_MORE_ZENER_DATABASE_FILE, index=False)
            os.remove(DIODES_MORE_MORE_ZENER_CSV_DATABASE_FILE)
            diodes_more_more_zener_df = load_and_cache("diodes_specs_more_more.pkl", DIODES_MORE_MORE_ZENER_DATABASE_FILE, True,
                lambda: pd.read_excel(DIODES_MORE_MORE_ZENER_DATABASE_FILE))
    ...
    all_dfs = (
        ...
        {"Data Line": diodes_dl_df, "Power Line": diodes_pl_df, "TSPDs": diodes_ts_df, "USB-C": diodes_usbc_df, "Zener": diodes_zener_df, "More Zeners": diodes_more_zener_df, "More more Zeners": diodes_more_more_zener_df},
    ...
    if mode == '2':
        ...
        for index, row in iterator:
            ...
            if is_zener_part_batch:
                comp_specs["Package"] = _canonical_to_ti_pkg(comp_specs.get("Canonical Package")) or normalize_package(original_display_package)
    ...
    if is_zener_part:
        ...
        comp_specs["Package"] = original_display_package
    else:
    ...
    for ti_spec in ti_alternatives_specs_list:
        ...
        # Prefer package that matches competitor canonical code
        if comp_canonical:
            for pkg in package_list:
                if _classify_ti_package(pkg, None) == comp_canonical:
                    chosen_pkg = pkg
                    break
        ...
        # No match — just use first package
        if not chosen_pkg:
