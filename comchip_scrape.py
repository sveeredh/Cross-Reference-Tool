...
def _parse_comchip_row(part_row, source_df_name, part_number):
    """
    Parses a row from the comchip CSV datasheet to extract key parameters.
    """
    specs_result = {
        ...
        "IEC 61000-4-2": "-", # Not present in the comchip CSV header
        "Capacitance": "-",
    ...
    try:
        ...
        # If values are not clean numbers, the default values ("-" and "1") will be used.
        pass
...
def fetch_comchip_specs_from_excel(part_number, comchip_specs_df):

    if part_number is None or comchip_specs_df is None or comchip_specs_df.empty:
        return None
    ...
    for col in comchip_specs_df.columns:
    ...
    if mfr_part_num_col:
        match_row_series = comchip_specs_df[comchip_specs_df[mfr_part_num_col].astype(str).str.strip().str.lower() == part_number.lower()]
        
        if not match_row_series.empty:
            part_row = match_row_series.iloc[0]
            matched_part_number = part_row[mfr_part_num_col]
            return _parse_comchip_row(part_row, "comchip_specs.csv", matched_part_number)
    ...
    if dk_part_num_col:
        ...
        match_row_series_dk = comchip_specs_df[
            comchip_specs_df[dk_part_num_col].astype(str).str.contains(search_pattern, case=False, na=False, regex=True)
        ]
        ...
        if not match_row_series_dk.empty:
            ...
            return _parse_comchip_row(part_row, "comchip_specs.csv", display_part_number)

    # If both searches fail, print the final message
    print(f"Part '{part_number}' not found in the local comchip file by Mfr P/N or DK P/N.")
    return None